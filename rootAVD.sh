...
#
# rootAVD - last refresh: May 2026
# Bundled Magisk:     v30.7 Stable (Feb 2026)
# Online menu source: https://x3rox.dev/extra/magisk/ (custom mirror)
#                     fallback: https://github.com/topjohnwu/magisk-files
# Compatible with:    Magisk v22.x .. v30.x (Stable & Canary)
##########################################################################################
...
CheckAvailableMagisks() {
	...
	# Custom mirror for stable.json / canary.json. Must end with a trailing slash.
	# Set to "" to fall back to the upstream topjohnwu/magisk-files repo.
	# The mirror only needs to host the two JSON channel files; the actual Magisk
	# APK is still pulled from the "link" URL inside the JSON (GitHub by default).
	local CUSTOM_MAGISK_FILES_URL="https://x3rox.dev/extra/magisk/"

	# Magisk Alpha (vvb2060) moved to Microsoft App Center / Telegram and no longer
	# publishes JSON channels on GitHub. The user can still drop a Magisk-vXX.YY.apk
	# (renamed to Magisk.zip) into the rootAVD folder and it will be used as the local version.
	local DLL_cnt=0
	...
	if [ -z $MAGISKVERCHOOSEN ]; then
		...
		if ("$AVDIsOnline"); then
			...
			# Prefer the custom mirror for the JSON channel files; fall back to GitHub raw
			if [[ -n "$CUSTOM_MAGISK_FILES_URL" ]]; then
				MAGISK_JSON_BASE="$CUSTOM_MAGISK_FILES_URL"
				echo "[*] Using custom Magisk channel mirror: $MAGISK_JSON_BASE"
			else
				MAGISK_JSON_BASE="$RAWGITHUB$TJWREPOURL""master/"
			fi

			FetchMagiskDLData $MAGISK_JSON_BASE "stable"
			FetchMagiskDLData $MAGISK_JSON_BASE "canary"
