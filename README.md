...
* Android 15 needs Magisk Version >= 28.x to be rooted (16k page size support)
* Android 16 (incl. QPR2) needs Magisk Version >= 30.7 to be rooted (new sepolicy binary format)
* Starting with API 34 (Android 14), Google Play system images use `init_boot.img` instead of `boot.img` for the ramdisk
	* The script auto-detects and patches the right image

### Notes on Magisk Versions
* The bundled `Magisk.zip` is **Magisk v30.7 Stable** (Feb 2026) — the last version that supports Android 16 QPR2
* When the AVD is online, the script always pulls the latest version via the menu (no need to update the bundled zip manually)
* Replace the Magisk.zip with the Flavour/Version of your choice for offline / pinned installs
* Tested Flavours are:
	* Magisk from [topjohnwu](https://github.com/topjohnwu/magisk-files) (Stable + Canary)
	* Kitsune Magisk / Delta from [huskydg](https://github.com/HuskyDG/magisk-files/releases)
	* ~~Magisk Alpha from vvb2060~~ — moved to Microsoft App Center / Telegram, no longer auto-fetched by the script
...
### Change Logs
#### [May 2026]
* [General] - Updated bundled Magisk to v30.7 Stable (Feb 2026 release)
* [rootAVD.sh] - Channel JSONs (`stable.json` / `canary.json`) now fetched from a custom mirror at `https://x3rox.dev/extra/magisk/`
	* The mirror only hosts the JSON metadata; the actual Magisk APK is still pulled from the GitHub URL inside the JSON
	* Set `CUSTOM_MAGISK_FILES_URL=""` in `CheckAvailableMagisks()` to revert to upstream `topjohnwu/magisk-files`
* [rootAVD.sh] - Removed unmaintained Magisk Alpha (vvb2060) channel from the menu
* [rootAVD.sh] - Compatibility tweaks for Magisk v27.x / v28.x / v29.x / v30.x
	* (handled automatically since binaries are extracted generically from `lib/$ABI/lib*.so`)
* [README.md] - Added compatibility notes for Android 14 (`init_boot.img`), 15 (16k page size), and 16 (Android 16 QPR2 sepolicy)
* [CompatibilityChart.md] - Refreshed table with Android 13 / 14 / 15 / 16 system images
